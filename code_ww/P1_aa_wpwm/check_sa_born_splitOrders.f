...
C     
     ...
     $ ,NCOLORCONNECTIONS)
      ...
      COMMON/AA_WPWM_COLOR_CORRELATION_MAPS/COLOR_CORRELATOR_TO_INDEX,
     $  INDEX_TO_COLOR_CORRELATOR
      ...
      DATA (CC_INDICES_OFFSET(I),I=1,CC_PERT_ORDER)/0,0/
      INTEGER N_CC_FOR_ORDER(CC_PERT_ORDER)
      DATA (N_CC_FOR_ORDER(I),I=1,CC_PERT_ORDER)/0,0/
...
C     Specify here that we want to compute all color correlators
      CALL AA_WPWM_SET_COLOR_CORRELATORS_TO_CONSIDER(-1,-1)
C     Turn off spin correlations for now
      CALL AA_WPWM_RESET_SPIN_CORRELATION_VECTORS()
      CALL AA_WPWM_SMATRIX_SPLITORDERS(P,MATELEMS)
      MATELEM=MATELEMS(0)
...
C     
     ...
     $ (2*NEXTERNAL-8)
      ...
      WRITE(*,*) '-----------------------------'
      WRITE(*,*) ''
      CALL AA_WPWM_GET_COLOR_CORRELATED_ME(COLOR_CORRELATED_EVALS)
...
C     Turn off color correlations
      CALL AA_WPWM_SET_COLOR_CORRELATORS_TO_CONSIDER(0,0)
      CALL AA_WPWM_RESET_SPIN_CORRELATION_VECTORS()
...
C        vector!       
        DO K=1,NVECTOR_TO_TRY_PER_LEG
          ...
          ENDDO
        ENDDO
        CALL AA_WPWM_SET_SPIN_CORRELATION_VECTORS(I
     $   ,NVECTOR_TO_TRY_PER_LEG,SCVECTORS)
      ENDDO
      CALL AA_WPWM_SMATRIX_SPLITORDERS(P,MATELEMS)
      MATELEM=MATELEMS(0)
      DO K=1,NSPLITORDERS+1
...
C     -----
      ...
      IF (PERT_ORDER.EQ.1) THEN
        CALL AA_WPWM_INDEX_TO_COLOR_CONNECTION_NLO(CC_INDEX,CC(1,1))
     ...
     $       //' test)'
          ENDIF
        ENDIF
      ELSEIF (PERT_ORDER.EQ.2) THEN
        CALL AA_WPWM_INDEX_TO_COLOR_CONNECTION_NNLO(CC_INDEX,CC(1,1)
     $   ,CC(1,2))
        IF (N_REPRESENTATIVES.NE.1) THEN
          IF (INCLUDED_IN_NEUTRALITY_TEST.NE.0) THEN
            WRITE(*,*) 'NNLO color connection #',CC_INDEX,' -> (',CC(1
     $       ,1),CC(2,1),CC(3,1),'),(',CC(1,2),CC(2,2),CC(3,2),') and'
     $       //' #representatives =',N_REPRESENTATIVES
          ELSE
            WRITE(*,*) 'NNLO color connection #',CC_INDEX,' -> (',CC(1
     $       ,1),CC(2,1),CC(3,1),'),(',CC(1,2),CC(2,2),CC(3,2),') and'
     $       //' #representatives =',N_REPRESENTATIVES,' (Not part of'
     $       //' colour neutrality test)'
          ENDIF
        ELSE
          IF (INCLUDED_IN_NEUTRALITY_TEST.NE.0) THEN
            WRITE(*,*) 'NNLO color connection #',CC_INDEX,' -> (',CC(1
     $       ,1),CC(2,1),CC(3,1),'),(',CC(1,2),CC(2,2),CC(3,2),')'
          ELSE
            WRITE(*,*) 'NNLO color connection #',CC_INDEX,' -> (',CC(1
     $       ,1),CC(2,1),CC(3,1),'),(',CC(1,2),CC(2,2),CC(3,2),') (Not'
     $       //' part of colour neutrality test)'
          ENDIF
        ENDIF
      ENDIF
