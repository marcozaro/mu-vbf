      SUBROUTINE AA_WPWM_ME_ACCESSOR_HOOK(P,HEL,USER_ALPHAS,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
CF2PY INTENT(OUT) :: ANS
      ...
      INTEGER AA_WPWM_DEN
      COMMON/AA_WPWM_IDEN/AA_WPWM_DEN

      INTEGER AA_WPWM_FL_FACTOR
      COMMON/AA_WPWM_FLAVOUR_FACTOR/AA_WPWM_FL_FACTOR
      AA_WPWM_FL_FACTOR = 1
...
C     ----------
      ...
      CALL AA_WPWM_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)

      END

      SUBROUTINE AA_WPWM_SMATRIX(P,ANS_SUMMED)
C     
...
C     
      ...
      CALL AA_WPWM_SMATRIX_SPLITORDERS(P,ANS)
      ANS_SUMMED=ANS(0)

      END

      SUBROUTINE AA_WPWM_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/AA_WPWM_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL AA_WPWM_SMATRIX(P,ANS)
      USERHEL=-1

      END

C     Give access to the helicity definition to the f2py API.
      SUBROUTINE AA_WPWM_GET_HELICITY_DEFINITIONS(NHEL_OUT)
      IMPLICIT NONE
      ...
      COMMON/AA_WPWM_BORN_HEL_CONFIGS/NHEL
...
CF2PY INTENT(OUT) :: NHEL_OUT
      ...
      SUBROUTINE AA_WPWM_SMATRIX_SPLITORDERS(P,ANS)
C     
...
C     
      ...
      COMMON/AA_WPWM_CHOSEN_BORN_SQSO/CHOSEN_SO_CONFIGS
C     
C     ARGUMENTS 
C     
      REAL*8 P(0:3,NEXTERNAL),ANS(0:NSQAMPSO)
...
C     
      ...
      COMMON/AA_WPWM_BORN_BEAM_POL/POLARIZATIONS
...
C      indices are which one to consider
     ...
     $ ,NSQAMPSO) / NCC_TIMES_NSQAMPSO*0.0D0 /
      COMMON/AA_WPWM_COLOR_CORRELATIONS/COLOR_CORRELATORS_TO_CONSIDER,
     $  COLOR_CORRELATED_EVALS

      INCLUDE 'aa_wpwm_spin_correlations.inc'
...
C     FUNCTIONS
C     
      LOGICAL AA_WPWM_IS_BORN_HEL_SELECTED
...
C      use to insert the extra spin. corr. runs.
     ...
     $       (.NOT.AA_WPWM_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
...
C            a normal run if not spin-correlations are active
     ...
     $         -1.OR.AA_WPWM_IS_BORN_HEL_SELECTED(IHEL))
              DO I=1,NEXTERNAL
...
C                 Spin correlation, enforce the polarization vector
     ...
     $               , I),J)
                ...
                ENDIF
              ENDDO
              CALL AA_WPWM_MATRIX(P ,HELICITIES(1),JC(1), T,
...
C              spin-correlation runs
     ...
     $           (CCI,I)/POLARIZATIONS(J,0)
      ...
      SUBROUTINE AA_WPWM_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     ----------
      ...
      SUBROUTINE AA_WPWM_MATRIX(P,NHEL,IC,RES, DO_INCLUDE_HEL_CONTRIB
     $ ,WEXT)
...
C     Amplitude(s) for diagram number 1
      ...
      CALL VVV1P0_2(W(1,1),W(1,3),GC_4,MDL_MW,MDL_WW,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL VVV1_0(W(1,2),W(1,4),W(1,5),GC_4,AMP(2))
      CALL VVS1_3(W(1,1),W(1,3),GC_93,MDL_MW,MDL_WW,W(1,5))
C     Amplitude(s) for diagram number 3
      CALL VVS1_0(W(1,2),W(1,4),W(1,5),GC_92,AMP(3))
      CALL VVV1P0_3(W(1,1),W(1,4),GC_4,MDL_MW,MDL_WW,W(1,5))
C     Amplitude(s) for diagram number 4
      CALL VVV1_0(W(1,2),W(1,5),W(1,3),GC_4,AMP(4))
      CALL VVS1_3(W(1,1),W(1,4),GC_92,MDL_MW,MDL_WW,W(1,5))
C     Amplitude(s) for diagram number 5
      CALL VVS1_0(W(1,2),W(1,3),W(1,5),GC_93,AMP(5))
...
C     Normalization
     ...
     $             *DCONJG(JAMP(I,N))/CC_DENOM(CCI,I)
      ...
      SUBROUTINE AA_WPWM_GET_ME(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      CALL UPDATE_AS_PARAM()
      IF (NHEL.NE.0) THEN
        CALL AA_WPWM_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL AA_WPWM_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE AA_WPWM_INITIALISE(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
...
C      automatically found
      ...
      LOGICAL FUNCTION AA_WPWM_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/AA_WPWM_BORN_HEL_CONFIGS/HELC
...
C     ----------
      ...
      AA_WPWM_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          AA_WPWM_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
...
C     --------------------------------------------------------------
      ...
      SUBROUTINE AA_WPWM_GET_COLOR_CORRELATED_ME(CORRELATED_MES)
      IMPLICIT NONE
...
C      indices are which one to consider
     ...
     $  COLOR_CORRELATED_EVALS
      ...
      SUBROUTINE AA_WPWM_GET_COLOR_CORRELATOR_ID(CONNECTION_INDEX_A,
...
CF2PY INTENT(OUT) :: ID
     ...
     $  CONNECTION_INDEX_B)
      ...
      SUBROUTINE AA_WPWM_GET_COLOR_CORRELATOR_FOR_ID(IDX,
     $  CONNECTION_INDEX_A, CONNECTION_INDEX_B)
      IMPLICIT NONE
...
CF2PY INTENT(OUT) :: CONNECTION_INDEX_B
     ...
     $  CORRELATOR_INDEX_TO_CONNECTION_INDEX
      ...
      SUBROUTINE AA_WPWM_GET_N_COLOR_CORRELATORS(N_COLOR_CORRELATORS)
...
CF2PY INTENT(OUT) :: N_COLOR_CORRELATORS
      ...
      SUBROUTINE AA_WPWM_ADD_COLOR_CORRELATORS_TO_CONSIDER
...
C     ----------
     ...
     $  SECOND_CONNECTION)
      ...
      SUBROUTINE AA_WPWM_SET_COLOR_CORRELATORS_TO_CONSIDER
     $ (FIRST_CONNECTION, SECOND_CONNECTION)
      IMPLICIT NONE
...
C     gl  
      DOUBLE PRECISION FUNCTION AA_WPWM_GET_CCBLO(LEG_A,LEG_B)
      IMPLICIT NONE
...
C     QAMPSO)      
     ...
     $   ))THEN
          AA_WPWM_GET_CCBLO = COLOR_CORRELATED_EVALS(I,1)
          EXIT
        ENDIF
      ENDDO
      RETURN
...
C     --------------------------------------------------------------
      ...
      DOUBLE PRECISION FUNCTION AA_WPWM_GET_KKBLO(PARENT_LEG,PB,KT)
      IMPLICIT NONE
      INTEGER I
      INTEGER NEXTERNAL
      PARAMETER (NEXTERNAL=4)
      ...
      CALL AA_WPWM_SET_SPIN_CORRELATION_VECTORS(PARENT_LEG
     $ ,NVECTOR_TO_TRY_PER_LEG,SCVECTORS)
      CALL AA_WPWM_SMATRIX_SPLITORDERS(PB,MATELEMS)
      AA_WPWM_GET_KKBLO = MATELEMS(0)
      ...
      SUBROUTINE AA_WPWM_SET_SPIN_CORRELATION_VECTORS(LEG_INDEX,
     $  N_VECTORS, VECTORS)
      IMPLICIT NONE
...
C      vectors of a leg again
     ...
     $   //' aa_wpwm_RESET_SPIN_CORRELATION_VECTORS() first.'
        STOP
      ENDIF
...
C      to SPIN_CORR_RUNS
      ...
      END SUBROUTINE AA_WPWM_SET_SPIN_CORRELATION_VECTORS

      SUBROUTINE AA_WPWM_RESET_SPIN_CORRELATION_VECTORS()
      IMPLICIT NONE
...
C     ----------
      ...
      END SUBROUTINE AA_WPWM_RESET_SPIN_CORRELATION_VECTORS

      SUBROUTINE AA_WPWM_GET_MAX_N_SPIN_CORR_VECTORS(MAX_N_SC_VECTORS)
      IMPLICIT NONE
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=4)
      INCLUDE 'aa_wpwm_spin_correlations.inc'
...
CF2PY INTENT(OUT) :: MAX_N_SC_VECTORS
      ...
      END SUBROUTINE AA_WPWM_GET_MAX_N_SPIN_CORR_VECTORS

      SUBROUTINE AA_WPWM_GET_MAX_N_SPIN_CORR_LEGS(MAX_N_SC_LEGS)
      IMPLICIT NONE
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=4)
      INCLUDE 'aa_wpwm_spin_correlations.inc'
...
CF2PY INTENT(OUT) :: MAX_N_SC_LEGS
      ...
      END SUBROUTINE AA_WPWM_GET_MAX_N_SPIN_CORR_LEGS

      SUBROUTINE AA_WPWM_GET_COLOR_CONNECTION_MAX_ORDER(MAX_ORDER)
      IMPLICIT NONE
...
CF2PY INTENT(OUT) :: MAX_ORDER
      ...
      END SUBROUTINE AA_WPWM_GET_COLOR_CONNECTION_MAX_ORDER
...
C     =========================================================
      ...
      SUBROUTINE AA_WPWM_GET_N_COLOR_CONNECTIONS_NLO(N_CC)
      IMPLICIT NONE
...
CF2PY INTENT(OUT) :: N_CC
      ...
      END SUBROUTINE AA_WPWM_GET_N_COLOR_CONNECTIONS_NLO

      SUBROUTINE AA_WPWM_APPLY_COLOR_CONNECTION_DEFINITIONS_NLO()
...
C      be zero
      ...
      COMMON/AA_WPWM_COLOR_CONNECTION_DEFINITIONS_NLO
     $ /COLOR_CONNECTIONS, COLOR_CONNECTION_INITIALIZED



      END SUBROUTINE AA_WPWM_APPLY_COLOR_CONNECTION_DEFINITIONS_NLO

      SUBROUTINE AA_WPWM_COLOR_CONNECTION_TO_INDEX_NLO(SPECIFIER_A,
     $  INDEX_OUT)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     ----------
      ...
      IF (.NOT.COLOR_CONNECTION_INITIALIZED) THEN
        ...
        CALL AA_WPWM_APPLY_COLOR_CONNECTION_DEFINITIONS_NLO()
      ENDIF
...
C     And return it now
      ...
      END SUBROUTINE AA_WPWM_COLOR_CONNECTION_TO_INDEX_NLO

      SUBROUTINE AA_WPWM_INDEX_TO_COLOR_CONNECTION_NLO(INDEX_IN
     $ ,SPECIFIER_A)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     
     ...
     $ ,PERT_ORDER,3)
      ...
      COMMON/AA_WPWM_CC_INDEX_TO_DEFINITION_NLO/CC_INDEX_TO_DEFINITION_
     $NLO
...
C     Return the color connection for this index
     ...
     $ -INDEX_NLO_OFFSET,1,3)
      ...
      END SUBROUTINE AA_WPWM_INDEX_TO_COLOR_CONNECTION_NLO

      FUNCTION AA_WPWM_GET_COLOR_CONNECTION_INDEX_NLO(SPECIFIER_A)
      IMPLICIT NONE
C     
C     ARGUMENTS 
C     
      ...
      INTEGER AA_WPWM_GET_COLOR_CONNECTION_INDEX_NLO
C     
C     LOCAL VARIABLES
C     
      INTEGER CC_INDEX
...
C     ----------
      ...
      CALL AA_WPWM_COLOR_CONNECTION_TO_INDEX_NLO(SPECIFIER_A, CC_INDEX)
      AA_WPWM_GET_COLOR_CONNECTION_INDEX_NLO = CC_INDEX

      END FUNCTION AA_WPWM_GET_COLOR_CONNECTION_INDEX_NLO

C     =========================================================
C     Mapping of NNLO color connections to a unique index
C     =========================================================

      SUBROUTINE AA_WPWM_GET_N_COLOR_CONNECTIONS_NNLO(N_CC)
      IMPLICIT NONE
      INTEGER N_CC_FOR_THAT_ORDER
      PARAMETER (N_CC_FOR_THAT_ORDER=0)
      INTEGER N_CC
CF2PY INTENT(OUT) :: N_CC
      N_CC = N_CC_FOR_THAT_ORDER
      END SUBROUTINE AA_WPWM_GET_N_COLOR_CONNECTIONS_NNLO

      SUBROUTINE AA_WPWM_APPLY_COLOR_CONNECTION_DEFINITIONS_NNLO()
C     This routine initialise the mapping of the color connections for
C      order NNLO
...
C      be zero
      DATA COLOR_CONNECTIONS/N_CC_DEFINITION_ENTRIES*0/
      DATA COLOR_CONNECTION_INITIALIZED/.FALSE./
      COMMON/AA_WPWM_COLOR_CONNECTION_DEFINITIONS_NNLO
     $ /COLOR_CONNECTIONS, COLOR_CONNECTION_INITIALIZED



      END SUBROUTINE AA_WPWM_APPLY_COLOR_CONNECTION_DEFINITIONS_NNLO

      SUBROUTINE AA_WPWM_COLOR_CONNECTION_TO_INDEX_NNLO(SPECIFIER_A
...
C     ----------
      ...
      IF (.NOT.COLOR_CONNECTION_INITIALIZED) THEN
        COLOR_CONNECTION_INITIALIZED = .TRUE.
        CALL AA_WPWM_APPLY_COLOR_CONNECTION_DEFINITIONS_NNLO()
      ENDIF
...
C     And return it now
     ...
     $ ,CC_SPECIFIER(3),CC_SPECIFIER(4))
      ...
      END SUBROUTINE AA_WPWM_COLOR_CONNECTION_TO_INDEX_NNLO

      SUBROUTINE AA_WPWM_INDEX_TO_COLOR_CONNECTION_NNLO(INDEX_IN
     $ ,SPECIFIER_A,SPECIFIER_B)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
      ...
      PARAMETER (NCOLORCONNECTIONS_NNLO=0)
      INTEGER INDEX_NNLO_OFFSET
      PARAMETER (INDEX_NNLO_OFFSET=0)
...
C     
     ...
     $ ,PERT_ORDER,3)
      ...
      COMMON/AA_WPWM_CC_INDEX_TO_DEFINITION_NNLO/CC_INDEX_TO_DEFINITION
     $_NNLO
...
C     Return the color connection for this index
     ...
     $ -INDEX_NNLO_OFFSET,2,3)
      ...
      END SUBROUTINE AA_WPWM_INDEX_TO_COLOR_CONNECTION_NNLO

      FUNCTION AA_WPWM_GET_COLOR_CONNECTION_INDEX_NNLO(SPECIFIER_A
     $ ,SPECIFIER_B)
      IMPLICIT NONE
C     
C     ARGUMENTS 
C     
      ...
      INTEGER AA_WPWM_GET_COLOR_CONNECTION_INDEX_NNLO
C     
C     LOCAL VARIABLES
C     
      INTEGER CC_INDEX
...
C     ----------
      ...
      CALL AA_WPWM_COLOR_CONNECTION_TO_INDEX_NNLO(SPECIFIER_A
     $ ,SPECIFIER_B, CC_INDEX)
      AA_WPWM_GET_COLOR_CONNECTION_INDEX_NNLO = CC_INDEX

      END FUNCTION AA_WPWM_GET_COLOR_CONNECTION_INDEX_NNLO
...
C     Set of functions to handle the array indices of the split orders
      ...
      INTEGER FUNCTION AA_WPWM_SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/AA_WPWM_AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER AA_WPWM_SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $   (ORDERINDEXB,I)
      ENDDO
      AA_WPWM_SQSOINDEX=AA_WPWM_SOINDEX_FOR_SQUARED_ORDERS(SQORDERS)
      END

      INTEGER FUNCTION AA_WPWM_SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/AA_WPWM_ALL_SQSPLITORDERS/SQSPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        AA_WPWM_SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping in function'
      WRITE(*,*) 'aa_wpwm_SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      SUBROUTINE AA_WPWM_GET_NSQSO_BORN(NSQSO)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE AA_WPWM_GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function aa_wpwm_GET_SQUARED_ORDERS_'
     $ //'FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE AA_WPWM_GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function aa_wpwm_GET_ORDERS_FOR_AMPS'
     $ //'OINDEX'
...
C      completeness
      INTEGER FUNCTION AA_WPWM_SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        AA_WPWM_SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function aa_wpwm_SOINDEX_FOR_AMPORDE'
     $ //'RS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     --------
      ...
      SUBROUTINE AA_WPWM_GET_SPLIT_ORDER_NAMES(SONAMES)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
