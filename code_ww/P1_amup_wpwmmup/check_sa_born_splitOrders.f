...
C     
     ...
     $ ,NCOLORCONNECTIONS)
      ...
      COMMON/AMUP_WPWMMUP_COLOR_CORRELATION_MAPS/COLOR_CORRELATOR_TO_IN
     $DEX, INDEX_TO_COLOR_CORRELATOR
      ...
      DATA (N_CC_FOR_ORDER(I),I=1,CC_PERT_ORDER)/0/
...
C     Specify here that we want to compute all color correlators
      CALL AMUP_WPWMMUP_SET_COLOR_CORRELATORS_TO_CONSIDER(-1,-1)
C     Turn off spin correlations for now
      CALL AMUP_WPWMMUP_RESET_SPIN_CORRELATION_VECTORS()
      CALL AMUP_WPWMMUP_SMATRIX_SPLITORDERS(P,MATELEMS)
      MATELEM=MATELEMS(0)
...
C     
     ...
     $ (2*NEXTERNAL-8)
      ...
      WRITE(*,*) '-----------------------------'
      WRITE(*,*) ''
      CALL AMUP_WPWMMUP_GET_COLOR_CORRELATED_ME(COLOR_CORRELATED_EVALS)
...
C     Turn off color correlations
      CALL AMUP_WPWMMUP_SET_COLOR_CORRELATORS_TO_CONSIDER(0,0)
      CALL AMUP_WPWMMUP_RESET_SPIN_CORRELATION_VECTORS()
...
C        vector!       
        DO K=1,NVECTOR_TO_TRY_PER_LEG
          ...
          ENDDO
        ENDDO
        CALL AMUP_WPWMMUP_SET_SPIN_CORRELATION_VECTORS(I
...
C     -----
      ...
      IF (PERT_ORDER.EQ.1) THEN
        CALL AMUP_WPWMMUP_INDEX_TO_COLOR_CONNECTION_NLO(CC_INDEX,CC(1
     $   ,1))
