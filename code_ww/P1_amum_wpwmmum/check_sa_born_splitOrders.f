...
C     
     ...
     $ ,NCOLORCONNECTIONS)
      ...
      COMMON/AMUM_WPWMMUM_COLOR_CORRELATION_MAPS/COLOR_CORRELATOR_TO_IN
...
C     Specify here that we want to compute all color correlators
      CALL AMUM_WPWMMUM_SET_COLOR_CORRELATORS_TO_CONSIDER(-1,-1)
C     Turn off spin correlations for now
      CALL AMUM_WPWMMUM_RESET_SPIN_CORRELATION_VECTORS()
      CALL AMUM_WPWMMUM_SMATRIX_SPLITORDERS(P,MATELEMS)
      MATELEM=MATELEMS(0)
...
C     
     ...
     $ (2*NEXTERNAL-8)
      ...
      WRITE(*,*) '-----------------------------'
      WRITE(*,*) ''
      CALL AMUM_WPWMMUM_GET_COLOR_CORRELATED_ME(COLOR_CORRELATED_EVALS)
...
C     Turn off color correlations
      CALL AMUM_WPWMMUM_SET_COLOR_CORRELATORS_TO_CONSIDER(0,0)
      CALL AMUM_WPWMMUM_RESET_SPIN_CORRELATION_VECTORS()
...
C        vector!       
        DO K=1,NVECTOR_TO_TRY_PER_LEG
          ...
          ENDDO
        ENDDO
        CALL AMUM_WPWMMUM_SET_SPIN_CORRELATION_VECTORS(I
...
C     -----
      ...
      IF (PERT_ORDER.EQ.1) THEN
        CALL AMUM_WPWMMUM_INDEX_TO_COLOR_CONNECTION_NLO(CC_INDEX,CC(1
     $   ,1))
