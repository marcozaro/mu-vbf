      SUBROUTINE AMUM_WPWMMUM_ME_ACCESSOR_HOOK(P,HEL,USER_ALPHAS,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
CF2PY INTENT(OUT) :: ANS
      ...
      INTEGER AMUM_WPWMMUM_DEN
      COMMON/AMUM_WPWMMUM_IDEN/AMUM_WPWMMUM_DEN

      INTEGER AMUM_WPWMMUM_FL_FACTOR
      COMMON/AMUM_WPWMMUM_FLAVOUR_FACTOR/AMUM_WPWMMUM_FL_FACTOR
      AMUM_WPWMMUM_FL_FACTOR = 1
...
C     ----------
      ...
      CALL AMUM_WPWMMUM_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)

      END

      SUBROUTINE AMUM_WPWMMUM_SMATRIX(P,ANS_SUMMED)
C     
...
C     
      ...
      CALL AMUM_WPWMMUM_SMATRIX_SPLITORDERS(P,ANS)
      ANS_SUMMED=ANS(0)

      END

      SUBROUTINE AMUM_WPWMMUM_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/AMUM_WPWMMUM_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL AMUM_WPWMMUM_SMATRIX(P,ANS)
      USERHEL=-1

      END

C     Give access to the helicity definition to the f2py API.
      SUBROUTINE AMUM_WPWMMUM_GET_HELICITY_DEFINITIONS(NHEL_OUT)
      IMPLICIT NONE
      ...
      COMMON/AMUM_WPWMMUM_BORN_HEL_CONFIGS/NHEL
...
CF2PY INTENT(OUT) :: NHEL_OUT
      ...
      SUBROUTINE AMUM_WPWMMUM_SMATRIX_SPLITORDERS(P,ANS)
C     
...
C     
      ...
      COMMON/AMUM_WPWMMUM_CHOSEN_BORN_SQSO/CHOSEN_SO_CONFIGS
C     
C     ARGUMENTS 
C     
      REAL*8 P(0:3,NEXTERNAL),ANS(0:NSQAMPSO)
...
C     
      ...
      COMMON/AMUM_WPWMMUM_BORN_BEAM_POL/POLARIZATIONS
...
C      indices are which one to consider
     ...
     $ ,NSQAMPSO) / NCC_TIMES_NSQAMPSO*0.0D0 /
      COMMON/AMUM_WPWMMUM_COLOR_CORRELATIONS/COLOR_CORRELATORS_TO_CONSI
     $DER, COLOR_CORRELATED_EVALS

      INCLUDE 'amum_wpwmmum_spin_correlations.inc'
...
C     FUNCTIONS
C     
      LOGICAL AMUM_WPWMMUM_IS_BORN_HEL_SELECTED

C     ----------
C     BEGIN CODE
C     ----------

      AMUM_WPWMMUM_DEN = IDEN
...
C      use to insert the extra spin. corr. runs.
     ...
     $       (.NOT.AMUM_WPWMMUM_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
...
C            a normal run if not spin-correlations are active
     ...
     $         -1.OR.AMUM_WPWMMUM_IS_BORN_HEL_SELECTED(IHEL))
              DO I=1,NEXTERNAL
...
C                 Spin correlation, enforce the polarization vector
     ...
     $               , I),J)
                ...
                ENDIF
              ENDDO
              CALL AMUM_WPWMMUM_MATRIX(P ,HELICITIES(1),JC(1), T,
...
C              spin-correlation runs
     ...
     $           (CCI,I)/POLARIZATIONS(J,0)
      ...
      SUBROUTINE AMUM_WPWMMUM_SMATRIXHEL_SPLITORDERS(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     ----------
      ...
      SUBROUTINE AMUM_WPWMMUM_MATRIX(P,NHEL,IC,RES,
     $  DO_INCLUDE_HEL_CONTRIB,WEXT)
...
C     FUNCTION
C     
      INTEGER AMUM_WPWMMUM_SQSOINDEX
C     
C     GLOBAL VARIABLES
C     
      INCLUDE 'coupl.inc'
...
C       First copy the external wavefunction with those provided
      ...
      CALL R_FFV1_2(W(1,2),W(1,1),GC_3,ZERO,ZERO,W(1,6))
      CALL VVV1P0_1(W(1,4),W(1,3),GC_4,ZERO,ZERO,W(1,7))
C     Amplitude(s) for diagram number 1
      CALL R_FFV1_0(W(1,6),W(1,5),W(1,7),GC_3,AMP(1))
      CALL VVV1P0_3(W(1,4),W(1,3),GC_71,MDL_MZ,MDL_WZ,W(1,8))
C     Amplitude(s) for diagram number 2
      CALL R_FFV2_4_0(W(1,6),W(1,5),W(1,8),GC_68,GC_77,AMP(2))
      CALL R_FFV2_1(W(1,5),W(1,3),GC_67,ZERO,ZERO,W(1,9))
C     Amplitude(s) for diagram number 3
      CALL R_FFV2_0(W(1,6),W(1,9),W(1,4),GC_67,AMP(3))
      CALL VVV1P0_2(W(1,1),W(1,3),GC_4,MDL_MW,MDL_WW,W(1,6))
      CALL R_FFV2_2(W(1,2),W(1,4),GC_67,ZERO,ZERO,W(1,10))
C     Amplitude(s) for diagram number 4
      CALL R_FFV2_0(W(1,10),W(1,5),W(1,6),GC_67,AMP(4))
      CALL R_FFV1P0_3(W(1,2),W(1,5),GC_3,ZERO,ZERO,W(1,11))
C     Amplitude(s) for diagram number 5
      CALL VVV1_0(W(1,11),W(1,4),W(1,6),GC_4,AMP(5))
      CALL R_FFV2_4P0_3(W(1,2),W(1,5),GC_68,GC_77,MDL_MZ,MDL_WZ,W(1,12)
     $ )
...
C     Amplitude(s) for diagram number 13
      CALL R_FFV2_0(W(1,2),W(1,9),W(1,6),GC_67,AMP(13))
      CALL R_FFV1_1(W(1,5),W(1,1),GC_3,ZERO,ZERO,W(1,6))
C     Amplitude(s) for diagram number 14
      CALL R_FFV2_0(W(1,10),W(1,6),W(1,3),GC_67,AMP(14))
C     Amplitude(s) for diagram number 15
      CALL R_FFV1_0(W(1,2),W(1,6),W(1,7),GC_3,AMP(15))
C     Amplitude(s) for diagram number 16
      CALL R_FFV2_4_0(W(1,2),W(1,6),W(1,8),GC_68,GC_77,AMP(16))
      CALL VVVV2P0_1(W(1,1),W(1,4),W(1,3),GC_5,ZERO,ZERO,W(1,6))
C     Amplitude(s) for diagram number 17
      CALL R_FFV1_0(W(1,2),W(1,5),W(1,6),GC_3,AMP(17))
      CALL VVVV5P0_4(W(1,1),W(1,4),W(1,3),GC_75,MDL_MZ,MDL_WZ,W(1,6))
C     Amplitude(s) for diagram number 18
      CALL R_FFV2_4_0(W(1,2),W(1,5),W(1,6),GC_68,GC_77,AMP(18))
...
C     Normalization
     ...
     $             ,M)
          ...
          ENDDO
          DO N = 1, NAMPSO
            SQSOIND = AMUM_WPWMMUM_SQSOINDEX(M,N)
     ...
     $             *DCONJG(JAMP(I,N))/CC_DENOM(CCI,I)
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_ME(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      CALL UPDATE_AS_PARAM()
      IF (NHEL.NE.0) THEN
        CALL AMUM_WPWMMUM_SMATRIXHEL(P, NHEL, ANS)
      ELSE
        CALL AMUM_WPWMMUM_SMATRIX(P, ANS)
      ENDIF
      RETURN
      ...
      SUBROUTINE AMUM_WPWMMUM_INITIALISE(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
...
C      automatically found
      ...
      LOGICAL FUNCTION AMUM_WPWMMUM_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      COMMON/AMUM_WPWMMUM_BORN_HEL_CONFIGS/HELC
...
C     ----------
      ...
      AMUM_WPWMMUM_IS_BORN_HEL_SELECTED = .TRUE.
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          AMUM_WPWMMUM_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
...
C     --------------------------------------------------------------
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_COLOR_CORRELATED_ME(CORRELATED_MES)
      IMPLICIT NONE
...
C      indices are which one to consider
     ...
     $DER, COLOR_CORRELATED_EVALS
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_COLOR_CORRELATOR_ID
     $ (CONNECTION_INDEX_A, CONNECTION_INDEX_B, ID)
      IMPLICIT NONE
...
CF2PY INTENT(OUT) :: ID
     ...
     $DEX, CORRELATOR_INDEX_TO_CONNECTION_INDEX
     ...
     $  CONNECTION_INDEX_B)
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_COLOR_CORRELATOR_FOR_ID(IDX,
...
CF2PY INTENT(OUT) :: CONNECTION_INDEX_B
     ...
     $DEX, CORRELATOR_INDEX_TO_CONNECTION_INDEX
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_N_COLOR_CORRELATORS
     $ (N_COLOR_CORRELATORS)
...
CF2PY INTENT(OUT) :: N_COLOR_CORRELATORS
      ...
      SUBROUTINE AMUM_WPWMMUM_ADD_COLOR_CORRELATORS_TO_CONSIDER
...
C     ----------
     ...
     $  SECOND_CONNECTION)
      ...
      SUBROUTINE AMUM_WPWMMUM_SET_COLOR_CORRELATORS_TO_CONSIDER
     $ (FIRST_CONNECTION, SECOND_CONNECTION)
      IMPLICIT NONE
...
C     gl  
      DOUBLE PRECISION FUNCTION AMUM_WPWMMUM_GET_CCBLO(LEG_A,LEG_B)
      IMPLICIT NONE
...
C     QAMPSO)      
     ...
     $   ))THEN
          AMUM_WPWMMUM_GET_CCBLO = COLOR_CORRELATED_EVALS(I,1)
          EXIT
        ENDIF
      ENDDO
      RETURN
...
C     --------------------------------------------------------------
      ...
      DOUBLE PRECISION FUNCTION AMUM_WPWMMUM_GET_KKBLO(PARENT_LEG,PB
     $ ,KT)
      IMPLICIT NONE
      INTEGER I
      INTEGER NEXTERNAL
      ...
      CALL AMUM_WPWMMUM_SET_SPIN_CORRELATION_VECTORS(PARENT_LEG
     $ ,NVECTOR_TO_TRY_PER_LEG,SCVECTORS)
      CALL AMUM_WPWMMUM_SMATRIX_SPLITORDERS(PB,MATELEMS)
      AMUM_WPWMMUM_GET_KKBLO = MATELEMS(0)
      ...
      SUBROUTINE AMUM_WPWMMUM_SET_SPIN_CORRELATION_VECTORS(LEG_INDEX,
     $  N_VECTORS, VECTORS)
      IMPLICIT NONE
...
C      vectors of a leg again
     ...
     $   //' amum_wpwmmum_RESET_SPIN_CORRELATION_VECTORS() first.'
        STOP
      ENDIF
...
C      to SPIN_CORR_RUNS
      ...
      END SUBROUTINE AMUM_WPWMMUM_SET_SPIN_CORRELATION_VECTORS

      SUBROUTINE AMUM_WPWMMUM_RESET_SPIN_CORRELATION_VECTORS()
      IMPLICIT NONE
...
C     ----------
      ...
      END SUBROUTINE AMUM_WPWMMUM_RESET_SPIN_CORRELATION_VECTORS

      SUBROUTINE AMUM_WPWMMUM_GET_MAX_N_SPIN_CORR_VECTORS
     $ (MAX_N_SC_VECTORS)
      IMPLICIT NONE
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=5)
...
CF2PY INTENT(OUT) :: MAX_N_SC_VECTORS
      ...
      END SUBROUTINE AMUM_WPWMMUM_GET_MAX_N_SPIN_CORR_VECTORS

      SUBROUTINE AMUM_WPWMMUM_GET_MAX_N_SPIN_CORR_LEGS(MAX_N_SC_LEGS)
      IMPLICIT NONE
      INTEGER    NEXTERNAL
      PARAMETER (NEXTERNAL=5)
      INCLUDE 'amum_wpwmmum_spin_correlations.inc'
...
CF2PY INTENT(OUT) :: MAX_N_SC_LEGS
      ...
      END SUBROUTINE AMUM_WPWMMUM_GET_MAX_N_SPIN_CORR_LEGS

      SUBROUTINE AMUM_WPWMMUM_GET_COLOR_CONNECTION_MAX_ORDER(MAX_ORDER)
      IMPLICIT NONE
...
CF2PY INTENT(OUT) :: MAX_ORDER
      ...
      END SUBROUTINE AMUM_WPWMMUM_GET_COLOR_CONNECTION_MAX_ORDER
...
C     =========================================================
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_N_COLOR_CONNECTIONS_NLO(N_CC)
      IMPLICIT NONE
...
CF2PY INTENT(OUT) :: N_CC
      ...
      END SUBROUTINE AMUM_WPWMMUM_GET_N_COLOR_CONNECTIONS_NLO

      SUBROUTINE AMUM_WPWMMUM_APPLY_COLOR_CONNECTION_DEFINITIONS_NLO()
...
C      be zero
      ...
      COMMON/AMUM_WPWMMUM_COLOR_CONNECTION_DEFINITIONS_NLO
     $ /COLOR_CONNECTIONS, COLOR_CONNECTION_INITIALIZED



      END SUBROUTINE AMUM_WPWMMUM_APPLY_COLOR_CONNECTION_DEFINITIONS_NL
     $O

      SUBROUTINE AMUM_WPWMMUM_COLOR_CONNECTION_TO_INDEX_NLO
     $ (SPECIFIER_A, INDEX_OUT)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      IF (.NOT.COLOR_CONNECTION_INITIALIZED) THEN
        ...
        CALL AMUM_WPWMMUM_APPLY_COLOR_CONNECTION_DEFINITIONS_NLO()
      ENDIF
...
C     And return it now
      ...
      END SUBROUTINE AMUM_WPWMMUM_COLOR_CONNECTION_TO_INDEX_NLO

      SUBROUTINE AMUM_WPWMMUM_INDEX_TO_COLOR_CONNECTION_NLO(INDEX_IN
     $ ,SPECIFIER_A)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     
     ...
     $ ,PERT_ORDER,3)
      ...
      COMMON/AMUM_WPWMMUM_CC_INDEX_TO_DEFINITION_NLO
...
C     Return the color connection for this index
     ...
     $ -INDEX_NLO_OFFSET,1,3)
      ...
      END SUBROUTINE AMUM_WPWMMUM_INDEX_TO_COLOR_CONNECTION_NLO

      FUNCTION AMUM_WPWMMUM_GET_COLOR_CONNECTION_INDEX_NLO(SPECIFIER_A)
      IMPLICIT NONE
C     
C     ARGUMENTS 
C     
      ...
      INTEGER AMUM_WPWMMUM_GET_COLOR_CONNECTION_INDEX_NLO
C     
C     LOCAL VARIABLES
C     
      INTEGER CC_INDEX
...
C     ----------
      ...
      CALL AMUM_WPWMMUM_COLOR_CONNECTION_TO_INDEX_NLO(SPECIFIER_A,
     $  CC_INDEX)
      AMUM_WPWMMUM_GET_COLOR_CONNECTION_INDEX_NLO = CC_INDEX

      END FUNCTION AMUM_WPWMMUM_GET_COLOR_CONNECTION_INDEX_NLO
...
C     Set of functions to handle the array indices of the split orders
      ...
      INTEGER FUNCTION AMUM_WPWMMUM_SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     
      ...
      COMMON/AMUM_WPWMMUM_AMPSPLITORDERS/AMPSPLITORDERS
C     
C     FUNCTION
C     
      INTEGER AMUM_WPWMMUM_SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
     ...
     $   (ORDERINDEXB,I)
      ENDDO
      AMUM_WPWMMUM_SQSOINDEX=AMUM_WPWMMUM_SOINDEX_FOR_SQUARED_ORDERS
     $ (SQORDERS)
      END

      INTEGER FUNCTION AMUM_WPWMMUM_SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      ...
      COMMON/AMUM_WPWMMUM_ALL_SQSPLITORDERS/SQSPLITORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSQSO
        ...
        ENDDO
        AMUM_WPWMMUM_SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping in function'
      WRITE(*,*) 'amum_wpwmmum_SOINDEX_FOR_SQUARED_ORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP

      END

      SUBROUTINE AMUM_WPWMMUM_GET_NSQSO_BORN(NSQSO)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE AMUM_WPWMMUM_GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX
     $ ,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function amum_wpwmmum_GET_SQUARED_OR'
     $ //'DERS_FOR_SOINDEX'
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE AMUM_WPWMMUM_GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function amum_wpwmmum_GET_ORDERS_FOR'
     $ //'_AMPSOINDEX'
...
C      completeness
      INTEGER FUNCTION AMUM_WPWMMUM_SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        AMUM_WPWMMUM_SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function amum_wpwmmum_SOINDEX_FOR_AM'
     $ //'PORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     --------
      ...
      SUBROUTINE AMUM_WPWMMUM_GET_SPLIT_ORDER_NAMES(SONAMES)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
