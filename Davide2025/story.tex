...
\\
{{\color{red}} Please pay attention that in some plots (Longitudinal=0, Right=R, Left=L) in some others (Longitudinal=L, Right=+ or p, Left=- or m) }
}
...
This good agreement deteriorates if the $\eta(t)$ distribution is considered, see Fig.~\ref{fig:cutsZZtt2} . \pnote{I fear there is a bug here, and EVA results could be in the partonic res frame. The same applies in all the other eta plots. The bug is not present for sure for the invariant mass because it is Lorentz-invariant. To be checked!} 
...
\pnote{I am not sure what conclude here. Seems like PDFs effects, i.e.~resummation is important.}
